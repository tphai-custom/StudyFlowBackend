...
async def list_drafts(
    ...
    drafts = await crud.list_drafts(db, current_user.id, draft_type=type)
...
async def create_draft(
    ...
    draft = await crud.create_draft(db, payload, current_user.id)
...
async def get_draft(
    ...
    if not draft or draft.owner_user_id != current_user.id:
...
async def update_draft(
    ...
    if not draft or draft.owner_user_id != current_user.id:
        raise HTTPException(status_code=404, detail="Draft not found")
    updated = await crud.update_draft(db, draft_id, payload)
    return _to_schema(updated).model_dump(by_alias=False)
...
async def finalize_draft(
    ...
    for item in (draft.items or []):
        ...
        task = await tasks_crud.create_task(db, task_payload, current_user.id)
...
async def delete_draft(
    ...
    if not draft or draft.owner_user_id != current_user.id:
        raise HTTPException(status_code=404, detail="Draft not found")
    await crud.delete_draft(db, draft_id)
